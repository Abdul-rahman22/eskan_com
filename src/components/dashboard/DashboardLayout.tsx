...
import { Building2, Plus, LayoutDashboard, List, Heart, Settings, LogOut, Menu, X, Home, User, ChevronLeft } from 'lucide-react';
...
const menuItems = [
  { path: '/dashboard', label: 'لوحة التحكم', icon: LayoutDashboard, description: 'مرحبا بك' },
  { path: '/dashboard/add-property', label: 'إضافة عقار', icon: Plus, description: 'أضف عقار جديد' },
  { path: '/dashboard/my-properties', label: 'عقاراتي', icon: List, description: 'عرض عقاراتك' },
  { path: '/dashboard/favorites', label: 'المفضلات', icon: Heart, description: 'العقارات المفضلة' },
  { path: '/dashboard/settings', label: 'الإعدادات', icon: Settings, description: 'إدارة الحساب' },
];
    ...
    <div className="min-h-screen bg-muted/30 dir-rtl">
      ...
      <header className="lg:hidden bg-primary text-primary-foreground sticky top-0 z-50">
        <div className="flex items-center justify-between h-14 px-4">
          <Button variant="ghost" size="icon" className="text-primary-foreground hover:bg-white/10" onClick={() => setSidebarOpen(true)}>
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <motion.aside initial={{ x: 100 }} animate={{ x: 0 }} exit={{ x: 100 }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} className="fixed right-0 top-0 bottom-0 w-72 bg-card z-50 lg:hidden shadow-2xl">
            <div className="flex flex-col h-full">
              ...
              <div className="p-4 border-b border-border flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-foreground text-sm">{profile?.fullname || user?.email?.split('@')[0]}</p>
              ...
              <div className="p-3 border-t border-border space-y-2">
                <Button variant="ghost" className="w-full justify-start gap-3 text-muted-foreground hover:text-foreground" onClick={() => { setSidebarOpen(false); navigate('/'); }}>
                  <Home className="h-5 w-5" />
                  Home
                </Button>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    {!active && <span className="text-xs opacity-0 group-hover:opacity-70 transition-opacity">{item.description}</span>}
                  </div>
